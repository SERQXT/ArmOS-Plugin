...
tier: 0
description: "Programmatically create and validate Domo beast modes (calculated fields) via the API — formula syntax, column schema fetching, validation endpoint, card-level vs dataset-level creation, curl output. Trigger with 'beast mode API', 'create beast mode via curl', 'beast mode validation endpoint', 'card-level beast mode', 'dataset-level beast mode', or other API-driven beast-mode creation requests. For authoring beast-mode formulas (without the API) use the beast-mode-writer skill."
maturity: alpha
audience: [code, delivery]
---
...
Authentication is handled automatically by the MCP layer. Do NOT create or manage tokens.
