...
name: appdb
tier: 0
description: "Toolkit-first AppDB document CRUD, query operators, collection wiring, and response unwrapping inside Domo custom apps using @domoinc/toolkit. Trigger with 'AppDBClient', 'AppDBClient.DocumentsClient', 'partialUpdate', 'doc.content', '@domoinc/toolkit AppDB', 'manifest.collections', or other in-app AppDB SDK questions. For provisioning AppDB via the MCP (creating collections, storing app settings) use the appdb-manager skill instead."
maturity: alpha
audience: [code]
---
