...
tier: 0
description: "Orchestrates a Domo custom app build with Remotion-compatible styling, sample data generation, and automatic 10-15 second demo video creation. Trigger with 'domo app with video', 'demo video build', 'remotion domo app', 'app build with demo'."
maturity: beta
deprecated: true
deprecation_note: "Deprecated — tutorial/video content belongs in documentation, not a skill. Build content superseded by procode-app-build (T2)."
audience: [code]
---

# Domo App + Demo Video Build Playbook

> **Publish path: `domo publish` is primary.** Caveats: don't run `domo login` interactively (use `mcp__armos-control__authenticate_instance`); never edit files under `~/.config/configstore/ryuu/`. If `domo publish` fails, fall back to `mcp__domo-publish__procode_publish`.
