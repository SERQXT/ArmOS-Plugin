...
tier: 0
description: "Convert SSR-heavy Lovable/v0/Next.js apps into client-only Domo custom apps, removing server-side rendering, API routes, and framework-specific dependencies while wiring Domo APIs and manifest. Trigger with 'migrate lovable', 'convert lovable', 'lovable to domo', 'v0 to domo', 'next.js to domo', 'convert SSR app'."
maturity: alpha
audience: [code]
---
...
> **Publish path: `domo publish` is primary.** Use `cd dist && domo publish` exactly as documented below. Two narrow caveats: (1) don't run `domo login` interactively — call `mcp__armos-control__authenticate_instance` to add a new instance via browser OAuth; (2) never edit files under `~/.config/configstore/ryuu/`. If `domo publish` fails (auth, oversize payload), the `mcp__domo-publish__procode_publish` MCP tool is the fallback — it wraps the CLI with manifest validation + a direct-API publish path.
