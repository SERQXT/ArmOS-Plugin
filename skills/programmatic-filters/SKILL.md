...
tier: 0
description: "Server-side programmatic filters and dataset switching for embedded Domo dashboards — OAuth embed token flow, standard/SQL filters, datasetRedirects for multi-tenant. Trigger with 'programmatic filter', 'embed token', 'server side filter domo', 'dataset redirect', 'embed auth domo'."
maturity: alpha
audience: [code]
---
