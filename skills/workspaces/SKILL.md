...
name: workspaces
tier: 0
description: "Reverse-engineered Domo Workspace REST API — add, remove, list, and search workspace content via MCP tools (or curl with a developer token as a fallback). Trigger with 'workspace api', 'domo workspace', 'add to workspace', 'workspace content', 'list workspaces'."
maturity: alpha
audience: [code]
---
...
> **Auth**: `X-Domo-Authentication: {SID}` (handled by MCP layer automatically)
...
Authentication is handled automatically by the MCP layer (Ryuu CLI session). You do NOT need to manage tokens. All requests are authenticated via the injected session credentials.

If you get a 401 error, call `authenticate_instance` to re-authenticate. Do NOT create developer tokens.
...
TOKEN = "YOUR_AUTH_TOKEN"  # Injected by MCP layer — do NOT create developer tokens
...
curl -X POST "https://your-instance.domo.com/api/nav/v1/workspaces/WORKSPACE_GUID/contents" \
  -H "X-Domo-Authentication: $DOMO_TOKEN" \
