...
name: performance
tier: 0
description: "Performance optimization rules for Domo custom app query shape, server-side aggregation strategy, column selection, payload minimization, and Query API limitations. Trigger with 'performance', 'optimize query', 'slow query', 'large dataset', 'reduce payload', 'query optimization'."
maturity: alpha
audience: [code]
---
...
```typescript
// TERRIBLE - Fetches 300k rows x 30 columns = 9M data points
const allData = await new Query()
  .select(['col1', 'col2', 'col3', ...]) // All columns
  .fetch('dataset');

// GOOD - Fetches 300k rows x 2 columns = 600k data points (93% reduction)
