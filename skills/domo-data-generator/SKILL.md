...
tier: 0
description: "Generate realistic sample datasets and upload to Domo using the datagen CLI — Salesforce, Google Analytics, financial, marketing, health data with cross-source entity integrity. Trigger with 'sample data', 'demo data domo', 'test data domo', 'datagen', 'domo data generator'."
maturity: alpha
audience: [code]
---
...
> **Note for ArmOS / Compass agents.** This skill is for the standalone
> `datagen` Python CLI from the `domo_data_generator` repository, which has
> **its own OAuth credential flow** independent of ArmOS's runtime.
> ArmOS itself uses Ryuu CLI sessions (`domo login -i {instance}.domo.com`)
> and injects the right env vars into MCP children automatically — do NOT
> tell users to edit `.env` files for normal ArmOS tooling. The `.env`
> setup below applies *only* when running `datagen` directly.
...
# Edit .env with your Domo credentials (datagen-specific — see note above)
```
