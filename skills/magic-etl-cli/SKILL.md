...
name: magic-etl-cli
tier: 0
description: "Build Magic ETL dataflows programmatically in Domo. Documents the complete JSON DAG action schema, every valid action type (aggregate, join, filter, formula, rollup, group-by), and the ILLEGAL action types (SaveToVault, SelectValues, RenameColumns) that trigger DP-0069 errors. Includes the community-domo-cli commands to submit and execute the DAG. Trigger with 'build a magic etl', 'dataflow with rollups', 'aggregate dataflow', 'dataflow action types', 'magic etl schema', 'output datasets from dataflow', 'monthly revenue trend', or any request to design or author a Magic ETL dataflow JSON body."
maturity: alpha
audience: [code]
---
