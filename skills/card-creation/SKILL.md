...
name: card-creation
tier: 0
description: "Complete Domo Product API reference for KPI card CRUD — all 207 chart types with override data, column mapping, beast modes, conditional formats, and code examples in Python and curl. Covers create, read, update, copy, delete operations with session auth, plus App Studio integration (create app, add card to page, layout positioning, writelock flow). Trigger with 'create card', 'card API', 'chart type', 'domo card', 'KPI card', 'badge chart', 'app studio card', 'add card to page', 'writelock', 'card layout'."
maturity: alpha
deprecated: true
deprecation_note: "Mined into 3 v2 T0s: card-builder (chart-type catalog + Pre-Render Validation), card-beastmode (beast mode authoring), card-conditional-format (conditional format rules). Body kept for reference. Do not add to routing maps."
audience: [code]
---
...
1. [Pre-Render Validation Checklist](#pre-render-validation-checklist)
2. [CRUD Endpoints](#1-crud-endpoints)
3. [Complete Body Schema](#2-complete-body-schema)
4. [Column Mapping](#3-column-mapping)
5. [Beast Modes](#4-beast-modes)
6. [Chart Types Reference](#5-chart-types-reference)
7. [Conditional Formats](#6-conditional-formats)
8. [Code Examples](#7-code-examples)
9. [Gotchas](#8-gotchas)
10. [App Studio Integration](#9-app-studio-integration)
...
|---|---|---|
| Profit <= Revenue | `sum(Profit) > sum(Revenue)` is impossible | Emit `DATA_ANOMALY`, skip ratio card, note in response |
...
| Heatmap height fit | `y + calculated_height > canvas_height` | Increase height or reposition card |

See the design skill (Section 0) for full cardinality gates and two-pass chart selection rules.
...
**Auth Header**: `X-Domo-Authentication: {SID}` (session auth from Ryuu CLI — handled automatically by the MCP layer)
...
API_KEY = "YOUR_AUTH_TOKEN"  # Injected by MCP layer — do not create developer tokens
PAGE_ID = "1227718422"
...
curl -X PUT "https://domo.domo.com/api/content/v3/cards/kpi?pageId=1227718422" \
  -H "X-Domo-Authentication: YOUR_SID" \
...
| **SID auth works for v3 cards** | `X-Domo-Authentication: {SID}` works for all `/content/v3/cards/kpi` endpoints (create, read, update). Auth is handled by the MCP layer. |
...
Auth:    X-Domo-Authentication: YOUR_SID  (handled by MCP layer)
...
## 9. App Studio Integration

When cards are placed inside App Studio apps rather than standard Domo pages, the workflow involves additional steps: creating the app container, attaching cards to the app's view page, and managing layout positioning through the writelock system.

### App Studio Architecture

App Studio apps are independent containers with their own view pages (not Domo dashboards). The key architectural differences from standard pages:

- Creating an app auto-creates a **view page** (`landingViewId`) and a **layout** for that page
- Cards are created globally (they exist outside the app), then **attached** to the app's view page
- Layout positions are set separately via the layout API after cards are on the page
- Editing the layout requires a **writelock** -- acquire it, make changes, release it

### ID Reference

| ID | Source | Used By |
|----|--------|---------|
| `dataAppId` | `POST /dataapps` response `.dataAppId` | All app updates, delete |
| `landingViewId` / `pageId` | `POST /dataapps` response `.landingViewId` or `.views[0].view.pageId` | Layout GET/PUT, add/remove cards |
| `layoutId` | `POST /dataapps` response `.views[0].layout.layoutId` | Layout writelock + PUT |
| `cardId` | `PUT /cards/kpi` response `.id` | Add to page, layout content, delete |

### 9.1 Create App

**Endpoint**: `POST /api/content/v1/dataapps`
...
```json
{
  ...
  "navOrientation": "TOP",
  "owners": [
    { "id": "{userId}", "type": "USER", "displayName": "{name}" }
  ]
}
```
...
`landingViewId` = the view page ID. Use this as `{pageId}` in all subsequent layout and card calls.

**Post-create sequence:** App create always triggers three follow-up calls:
1. `PUT /api/content/v1/dataapps/{dataAppId}/persistSettings` -- flush settings
2. `PUT /api/content/v1/dataapps/{dataAppId}` -- update metadata
3. `PUT /api/content/v1/dataapps/{dataAppId}/navigation/reorder` -- set nav order

### 9.2 Initialize Card from Dataset

Called when selecting a dataset in the card editor. Returns column metadata and a default definition.

**Endpoint**: `PUT /api/content/v3/cards/kpi/definition`
...
**Response** (200, ~340ms): Returns the dataset's columns and a default card definition with empty subscriptions. Use `columns[]` from this response to populate the card's subscription columns.

### 9.3 Create Card (App Studio Context)

Cards for App Studio use the same `PUT /api/content/v3/cards/kpi` endpoint documented in Section 1. The only difference: in App Studio you do NOT pass `?pageId=` because the card is attached to the page separately (see 9.4). When the card is created through the App Studio editor rather than the standard page flow, the endpoint operates as create-only with no `pageId` query parameter.

For the complete card body schema, column mapping, and chart type options, refer to Sections 2-5.

#### Additional Column Properties (App Studio)

**Column `format` object** (apply to any numeric VALUE column to control display):

```json
{ "column": "Sales", "aggregation": "SUM", "mapping": "VALUE",
  "format": {
    "type": "currency",
    "format": "$###,##0.00",
    "commas": true,
...
| `type` | `format` pattern | Use for |
|--------|-----------------|---------|
| `"currency"` | `"$###,##0.00"` | Dollar amounts (Sales, Revenue, Profit) |
| `"number"` | `"####"` | Counts, quantities (no currency symbol) |
| `"abbreviated"` | `"#A"` | Large numbers shown as 1.2M, 450K |

**Column `alias`** (renames the column label in chart legends and tooltips):

```json
{ "column": "Sales", "aggregation": "SUM", "mapping": "VALUE", "alias": "Total Revenue ($)" }
```

#### `dateTimeElement` values for `dateGrain`

`YEAR`, `QUARTER`, `MONTH`, `WEEK`, `DAY`

### 9.4 Add Cards to App Page

Attaches created cards to the app's view page.

**Endpoint**: `PUT /api/content/v1/cards/bulk/pages`

**Body:**
```json
{
  "cardIds": [4068294, 76254407, 547254106],
  "destinationPageIds": [475064280]
}
```

Use `landingViewId` as `destinationPageIds[0]`. Average timing: ~1334ms.

### 9.5 Position Cards with Layout

After adding cards to the page, set their positions. This requires the layout writelock system.

**Step 1 -- Acquire writelock:**
```
PUT /api/content/v4/pages/layouts/{layoutId}/writelock
```
No body required. Must be acquired before any layout PUT.
...
`contentKey` is a 0-based index across all content items. Each card in the layout needs a unique sequential `contentKey`.

**Step 3 -- Release writelock:**
```
DELETE /api/content/v4/pages/layouts/{layoutId}/writelock
```

**Keepalive** (if layout editing takes >30s):
```
PUT /api/content/v4/pages/layouts/{layoutId}/writelock/heartbeat
```

### 9.6 Remove Cards from Page

Detaches cards from a page (cards still exist globally).

**Endpoint**: `DELETE /api/content/v1/cards/bulk/pages`

**Body:**
```json
{
  "cardIds": [4068294, 4068295],
  "pageIds": [475064280]
}
```

**Note:** This endpoint may return 400 in some configurations. **Workaround:** Delete the card permanently with `DELETE /api/content/v1/cards/bulk?cardIds=...`, then update the layout content array to omit the removed card. The layout PUT is authoritative for what appears on the page.

### 9.7 Bulk Delete Cards

Deletes cards from Domo entirely (not just from the page).

**Endpoint**: `DELETE /api/content/v1/cards/bulk?cardIds={id1},{id2},{id3}`

Card IDs are passed as a comma-separated query parameter. Request body is ignored. Response: 200, empty body.

### 9.8 Delete App

**Endpoint**: `DELETE /api/content/v1/dataapps/{dataAppId}`

No request body. Response: 200. Average timing: ~217ms.

**Important:** This deletes the app and its view pages, but **not** the cards themselves -- cards must be deleted separately with `DELETE /api/content/v1/cards/bulk` if needed.

### 9.9 Supporting Endpoints (App Studio)

| Endpoint | Method | Purpose |
|----------|--------|---------|
| Get app | `GET /api/content/v1/dataapps/{dataAppId}` | Current app state with views and layout references |
| List apps | `GET /api/content/v1/dataapps` | All accessible apps |
| Get chart options | `GET /api/content/v1/cards/kpi/{chartType}/options` | Chart-specific styling config |
| Render saved card | `PUT /api/content/v1/cards/kpi/{cardId}/render` | Re-render after adding to page |
| Get dataset schema | `GET /api/data/v3/datasources/{datasetId}/schema` | Column list with types |
| Preview card | `PUT /api/content/v3/cards/kpi/render/preview` | Validate config, returns base64 PNG |

### Full App Studio Create Sequence (End-to-End)

```
1.  POST   /api/content/v1/dataapps                            -> dataAppId, landingViewId, layoutId
2.  PUT    /api/content/v1/dataapps/{dataAppId}/persistSettings
3.  PUT    /api/content/v1/dataapps/{dataAppId}
4.  PUT    /api/content/v4/pages/layouts/{landingViewId}/writelock
5.  PUT    /api/content/v1/dataapps/{dataAppId}/navigation/reorder

    (For each card:)
6.  PUT    /api/content/v3/cards/kpi/definition                 -> column list (init)
7.  PUT    /api/content/v3/cards/kpi                            -> cardId

    (Add all cards to page:)
8.  PUT    /api/content/v1/cards/bulk/pages                     {cardIds, destinationPageIds:[landingViewId]}

    (Position cards:)
9.  PUT    /api/content/v4/pages/layouts/{layoutId}/writelock
10. PUT    /api/content/v4/pages/layouts/{layoutId}             {content: [{cardId, contentKey, ...}]}
11. DELETE /api/content/v4/pages/layouts/{layoutId}/writelock
```

### Full App Studio Delete Sequence

```
1. DELETE /api/content/v1/cards/bulk?cardIds={id1},{id2}       -> delete cards
2. DELETE /api/content/v1/dataapps/{dataAppId}                 -> delete app
```

### App Studio Timing Reference

| Endpoint | Avg Timing |
|----------|-----------|
| `POST /api/content/v1/dataapps` | ~190ms |
| `PUT /api/content/v3/cards/kpi/definition` | ~340ms |
| `PUT /api/content/v3/cards/kpi` | ~687ms |
| `PUT /api/content/v1/cards/bulk/pages` | ~1334ms |
| `PUT /api/content/v4/pages/layouts/{id}` | ~302ms |
| `DELETE /api/content/v4/pages/layouts/{id}/writelock` | ~169ms |
| `DELETE /api/content/v1/cards/bulk` | ~171ms |
| `DELETE /api/content/v1/dataapps/{id}` | ~217ms |

### App Studio Gotchas

| Gotcha | Details |
|--------|---------|
| **`layoutId` != `landingViewId`** | These are two different IDs. `landingViewId` is the view/page ID. `layoutId` is a separate identifier for the grid layout object. The layout API uses `layoutId` -- NOT `landingViewId`. Capture `layoutId` from the app creation response at `views[0].layout.layoutId`. |
| **`PUT /cards/kpi` is create-only in App Studio** | The endpoint always creates a new card regardless of whether you pass `id` in the body. To replace a card: delete with `DELETE /cards/bulk?cardIds={id}`, create fresh with `PUT /cards/kpi`, add to page, update layout content. |
| **Writelock required for layout changes** | Acquire before PUT, release after. If the lock holds >30s without a heartbeat, it may expire. |
| **Cards exist independently of apps** | `PUT /cards/kpi` creates globally. `PUT /cards/bulk/pages` attaches. `DELETE /cards/bulk/pages` detaches. `DELETE /cards/bulk` deletes permanently. Deleting an app does NOT delete its cards. |
| **contentKey must be sequential from 0** | Each item in layout `content[]` needs a unique `contentKey` starting at 0. This is an index, not a position. |
| **`virtualAppendix: true` on template items** | When writing a fresh layout, set `virtualAppendix: false` on positioned cards for custom placement. |
| **Date grain uses a virtual column name** | When using `dateGrain`, the ITEM column must reference the **virtual calendar column** (e.g. `"CalendarQuarter"`), not the raw date field. The `dateGrain.column` references the source date field. Using the raw date column as ITEM creates a card that silently fails to render. |
| **`badge_xybubble` needs `XTIME` mapping** | Unlike other charts that use `ITEM`, bubble charts use `XTIME` for the X measure, `VALUE` for Y, and `SERIES` for grouping. Requires 8+ distinct SERIES values to be meaningful. |
| **`badge_heatmap` height must fit row count** | Formula: `height = max(12, ceil(distinct_series_count / 2) + 6)`. Verify `y + height <= canvas_height` before placing. |

### Chart Overrides Reference (App Studio)

The `overrides` object inside `charts.main` controls axis labels, legend, fonts, and visual styling. Key overrides for dashboard quality:
...
Available tokens: `%_VALUE`, `%_PERCENT_OF_TOTAL`, `%_CATEGORY_NAME`, `%_SERIES_NAME`, `%_GRAPH_TOTAL`, `%_PERCENT_OF_CATEGORY`.

**Legend:**
```json
"overrides": {
  "lrg_legend_position": "Top",
  "details_legend_position": "Right",
  "legend_title_text": "Region"
}
```

For the complete override reference by chart type group, see Section 5.

---

## Source

Adapted from [stahura/domo-ai-vibe-rules](https://github.com/stahura/domo-ai-vibe-rules) -- Riley Stahura's Domo AI skills collection. App Studio integration content derived from 318 captured network requests (2026-04-11).
