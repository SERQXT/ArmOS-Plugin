...
tier: 0
description: "Build and embed pro-code JavaScript custom apps inside Domo App Studio pages. Covers decision routing between native cards and pro-code, build workflow, canvas integration, page-level filter propagation (domo.onFiltersUpdated), App Studio variable integration (domo.onVariablesUpdated), theme alignment, banner patterns, React+Recharts chart templates, and Chart.js vanilla patterns. Trigger with 'pro-code', 'custom app', 'pro code editor', 'embed custom app', 'recharts', 'domo custom visualization'."
maturity: beta
deprecated: true
deprecation_note: "Superseded by procode-app-build (T2) in the v2 rebuild. Decision routing superseded by procode-app-decide (T1)."
audience: [code]
---
...
> **Publish path: `domo publish` is primary.** Use the CLI flow documented here. Caveats: don't run `domo login` interactively (use `mcp__armos-control__authenticate_instance`); never edit files under `~/.config/configstore/ryuu/`. If `domo publish` fails, `mcp__domo-publish__procode_publish` is the fallback (wraps CLI + adds API-fallback + manifest validation).
