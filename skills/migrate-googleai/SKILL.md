...
tier: 0
description: "Convert Google AI Studio-origin apps (Cloud Run, Gemini API, Dockerfile-based) to static Domo App Platform custom apps with correct dist/ deploy contract, relative base paths, and no server runtime. Trigger with 'google ai studio', 'migrate google', 'cloud run to domo', 'gemini app', 'convert ai studio'."
maturity: alpha
audience: [code]
---
...
> **Publish path: `cd dist && domo publish` is primary.** Two narrow caveats: don't run `domo login` interactively (use `mcp__armos-control__authenticate_instance` for new instances); never edit files under `~/.config/configstore/ryuu/`. If `domo publish` fails, fall back to `mcp__domo-publish__procode_publish`.
