...
name: fileset-api
tier: 0
description: "Build Domo custom apps that use FileSets for file storage, upload via FormData/fetch, download as blob, file listing via POST /files/search, AI document queries via /query endpoint, and fileset CRUD -- all via fetch('/api/files/v1/') with no manifest.json entry required. Trigger with 'fileset', 'file upload', 'file download', 'file storage', '/api/files/v1', 'document search', 'file browser'."
maturity: alpha
audience: [code]
---
